...
$('#list_surat').on('click', '#confirm', function () {
	...
	$.ajax({
		...
		success: function (result) {
			response = JSON.parse(result)
			$("#idData").val(response.id)
			$("#id_aksi").val(response.aksi_surat)
			$("#akiKirimNa").val(response.aksi_kirim)
...
function getAksi() {
	...
	$.ajax({
		...
		success: function (result) {
			let response = jQuery.parseJSON(JSON.stringify(result));
			response.forEach(item => {
				$('#id_aksi').append("<option disabled value=" + item.id + ">" + item.aksi + "</option>");
			})
		}
	})
}
...
function nomorSurat() {
	$.ajax({
		dataType: "json",
		url: baseUrl + 'admin/surat/masuk/aksi/getNomorUrut/1',
		success: function (result) {
			let response = jQuery.parseJSON(JSON.stringify(result))
