...
$('.show-data').on('click', function (evt) {
    ...
    PageFile(1, jenis, '', val);
})
function PageFile(page = 1, jenis = '', q = '', offset = 10) {
    let data = { page: page };
    if (q != '') data.q = q;
    ...
    $.ajax({
        url: baseUrl + 'admin/dokumen-saya/getfile',
        data: data,
        type: 'GET',
        dataType: 'json',
        beforeSend: function () {
...
function PageJenis(page = 1, id = '') {
    ...
    if (id != '') data.id = id;
    $.ajax({
        url: baseUrl + 'admin/dokumen-saya/getjenis',
        data: data,
        type: 'GET',
        dataType: 'json',
        beforeSend: function () {
...
function DisplayDataFile(data) {
    ...
    for (let x = 0; x < data.length; x++) {
        ...
        judul = data[x]['judul'];
        if (judul.length > 50)
            judul = data[x]['judul'].substr(0, 50) + '...';
        html += `
            <tr>
                ...
                <td class="nunito-400 py-2 item-td">
                    <div class="d-flex justify-content-start">
                        ...
                        <div class="ml-3">
                            <span class='pt-2'>
                                <a target='_blank' class='nunito-700 text-primary text-size--1' onclick='event.stopPropagation()' href='${url + 'upload/documents/' + data[x]['ori_file']}'>${judul}</a>
                            </span><br />
                ...
                <td class="nunito-400 item-td text-center">${data[x]['tipe_file']}</td>
                <td class="nunito-700 item-td text-center">${data[x]['akses_file'] == '' ? '<span class="text-success">Public</span>' : '<span class="text-info">Private</span>'}</td>
...
function UbahNama(ID, index) {
    $(`#ubah-${ID}`).fireModal({
        ...
        created: function (modal) {
            $.ajax({
                url: url + 'admin/dokumen-saya/getfilename',
                data: { id: ID },
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    if (response.status_code == 200) {
                        const html = `
                        <form action='${url + 'admin/dokumen-saya/storeubah'}' action='POST' autocomplete='off' id='StoreUbah'>
...
$(document).delegate('#StoreUbah', 'submit', function (evt) {
    ...
    $.ajax({
        url: url + 'admin/dokumen-saya/storeupdate',
        type: 'POST',
        data: Data,
        dataType: 'json',
        processData: false,
...
function Unduh(id) {
    document.location = url + 'admin/dokumen-saya/unduh-file?file=' + id
}
