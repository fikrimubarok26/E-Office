...
class M_document extends CI_Model
{
    ...
    function GetDataFile($data = null, $page = 1, $q = null, $limit = 10)
    {
        try {
            ...
            // echo $start . ' - ' . $end;
            // exit(1);
            $ReadFile = $this->GetData('doc_file', $where, [
                'start' => $start,
                'offset' => $limit
            ], $q);
            ...
            $SettingPagination['total_rows'] = $q == null ? $this->GetData('doc_file', $where)->num_rows() : $ReadFile->num_rows();
    ...
    function GetData($table, $data = null, $page = null, $q = null)
    {
        $this->db->select('doc_file.id,judul,tipe_file,ori_file,last_update,akses as akses_file');
        $this->db->from($table);
        if (is_array($data) && count($data) > 0) {
            foreach ($data as $key => $list) :
                $tampung[$key] = $list;
            endforeach;
            $this->db->where($tampung);
        }
        $this->db->join('doc_jenis_file', 'doc_file.id_jenis = doc_jenis_file.id', 'inner');
        if ($page != null)
            $this->db->limit($page['offset'], $page['start']);
        if ($q != null) {
            $counter = 1;
