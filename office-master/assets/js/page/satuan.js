...
$('#list_satuan').on('click', '#edit', function () {
	...
	$.ajax({
		...
		success: function (result) {
			...
			$("#kode_satuan1").val(response.kode_satuan)
			$("#satuan1").val(response.satuan)
...
$("#btnAllOff").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dinon-aktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/satuan/allOff`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllOn").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan diaktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/satuan/allOn`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllHapus").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dihapus  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/satuan/allDelete`,
					data: {
						id: dipilih
					},
					type: "post",
