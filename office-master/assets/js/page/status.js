$(document).ready(function () {
	table = $('#list_status').DataTable({
		"processing": true,
		"serverSide": true,
		"order": [],

		"ajax": {
			"url": baseUrl + 'admin/status/data',
			"type": "POST",
			"complete": function () {
				cekPilihanNa()
			},
...
$('#list_status').on('click', '#edit', function () {
	let id = $(this).data('id');
	$.ajax({
		url: baseUrl + 'admin/status/get/' + id,
		type: "GET",
		success: function (result) {
			response = JSON.parse(result)
			$("#idData").val(response.id)
			$("#kode_status1").val(response.kode_status)
...
$('#list_status').on('click', '#delete', function () {
	let id = $(this).data('id');
	confirmSweet("Data akan terhapus secara permanen !")
		.then(result => {
			if (result) {
				$.ajax({
					url: baseUrl + 'admin/status/delete/' + id,
					type: "GET",
					success: function (result) {
						response = JSON.parse(result)
...
$("#formAddStatus").submit(function (e) {
	e.preventDefault();
	$.ajax({
		url: baseUrl + "admin/status/insert",
		type: "post",
...
$("#btnAllOff").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dinon-aktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/status/allOff`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllOn").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan diaktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/status/allOn`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllHapus").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dihapus  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/status/allDelete`,
					data: {
						id: dipilih
					},
					type: "post",
